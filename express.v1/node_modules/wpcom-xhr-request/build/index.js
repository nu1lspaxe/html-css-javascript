...
var defaults = {
	...
	onStreamRecord: function onStreamRecord() {},
...
var sendResponse = function sendResponse(req, settings, fn) {
	var isEnvelopeMode = settings.isEnvelopeMode,
	    ...
	    onStreamRecord = settings.onStreamRecord;
	...
	req.end(function (error, response) {
		...
		var processResponseInStreamMode = shouldProcessInStreamMode(headers['content-type']);

		if (!req.xhr) {
			// node
			if (processResponseInStreamMode) {
				var _error = new Error('stream mode processing is not yet implemented for Node.js');
				return fn(_error, body, headers);
			}
		}
...
function enableStreamModeProcessing(req, onStreamRecord) {
	...
	req.xhr.addEventListener('progress', function (_ref) {
		...
		while (true) {
			var stop = target.response.indexOf('\n', start);
